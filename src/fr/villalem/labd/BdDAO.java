...
public class BdDAO {
	...
	public BdDAO(Connexion uneCo){
        ...
        */
        ...
        public String getCommentService(String service) throws SQLException{
            String quest = "SELECT descriptif FROM OptionsServices WHERE libelle = '"+service+"'AND nature='services'";
            rs = co.query(quest);
            if(rs.next()){
                String comment = rs.getString("descriptif");
                return comment;
        ...
        public ResultSet getReservations() {
            String request = "SELECT dateDebut, dateFin, nbPersonnes, validation FROM Reservations";
            rs = co.query(request);
            return rs;
        }
     ...
     * @param cal
        ...
        public ResultSet getReservationsSemaine(Calendar cal) {
            ...
            String request = "SELECT dateDebut, dateFin, nombrePersonnes, valide FROM Reservations WHERE dateDebut > '" + dateDebut +"' AND dateFin < '" + dateFin + "'";
            rs = co.query(request);
            return rs;
        }
